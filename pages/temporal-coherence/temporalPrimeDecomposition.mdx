...
`D = ∏_(i=1)^n ∏_(p∈P_T^(i)) p^(φ_T^(i)(D)(p))`

Where P_T^(i) represents the set of temporal primes at timescale τ_i, and φ_T^(i) is the corresponding mapping.
...
`Φ_T(D) = ∑_p φ_T(D)(p) δ(f - f_p)`

Where f_p = 1/τ_p is the characteristic frequency of prime p, and Φ_T(D) is the temporal prime spectrum.
...
`p_osc(t) = F(t/τ_p) where F(t + 1) = F(t)`
...
Formal representation:
`p_growth(t) = G(t/τ_p) where lim_(t→∞)G(t) = K`
...
Formal representation:
`p_trans(t) = T((t-t_0)/τ_p) where lim_(t→-∞)T(t) = A, lim_(t→∞)T(t) = B`
...
Formal representation:
`p_fluct(t) = S(t/τ_p) where S has characteristic statistical properties`
...
Formal representation:
`p_attr(x⃗) = A(x⃗) where A defines an irreducible attractor structure`
