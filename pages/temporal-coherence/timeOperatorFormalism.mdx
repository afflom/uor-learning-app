...
$$\mathcal{T}_{\Delta t}[\phi_Q] = e^{-i\hat{H}\Delta t/\hbar} \phi_Q$$

Where $\hat{H}$ is the Hamiltonian operator in prime coordinate representation.
...
$$\mathcal{T}_{\Lambda}[\phi_R] = U(\Lambda)\phi_R$$

Where $\Lambda$ is a Lorentz transformation and $U(\Lambda)$ is its representation in prime coordinate space.
...
$$\mathcal{T}_{\Delta t}[\phi_C] = \mathcal{F}(\phi_C, \nabla \phi_C, \mathcal{E}[\phi_C])$$

Where $\mathcal{F}$ captures the system's dynamics and $\mathcal{E}$ represents environmental coupling.
