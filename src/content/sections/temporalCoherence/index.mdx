...
The Temporal Coherence framework introduces the time operator $\mathcal{T}$ that acts on the prime coordinate representation of an object:

$$\mathcal{T}[\phi(O)] = \phi(O(t))$$

Where $\phi(O)$ is the prime coordinate representation of object $O$, and $\phi(O(t))$ represents its time-evolved state.

The temporal coherence between two time-separated states is measured by:

$$C_T(O(t_1), O(t_2)) = f(\phi(O(t_1)), \phi(O(t_2)))$$
...
$$D = \prod_{p \in \mathcal{P}_T} p^{\phi_T(D)(p)}$$

Where $\mathcal{P}_T$ is the set of temporal prime elements and $\phi_T(D)$ maps the process to its temporal prime coordinates.
